...
- **Skim Mode**: Quick search with full abstracts (no truncation)
- **Deep Read Mode**: Full PDF text extraction and structured analysis
- **Phase-aware**: Literature retrieval runs throughout all research phases
...
1. **Multi-Database Search with paper_search.py**:
   ...
   **Options:**
   - `--sources LIST`: Comma-separated sources (semantic, arxiv, crossref)
   - `--max N`: Max results per source (default: 10)
   - `--year YEAR`: Year filter for Semantic Scholar (e.g., '2020', '2020-2024')
   - `--format FORMAT`: Output format (markdown, json)
   - `--output FILE`: Save results to file
   - `--dedupe`: Remove duplicate papers
   - `--pdf-only`: Only show papers with PDF links

   **Supported Databases:**
   - **Semantic Scholar**: 200M+ papers, cross-disciplinary, citation graphs
   - **arXiv**: Preprints in physics, math, CS, q-bio
   - **CrossRef**: DOI database with metadata for 100M+ works
...
**Scripts:**
- `scripts/paper_search.py`: Multi-database search (Semantic Scholar, arXiv, CrossRef) with PDF links. **NEW: Full abstracts by default (no truncation)**
- `scripts/deep_read.py`: **NEW** Full PDF text extraction and structured deep reading notes
...
## Continuous Literature Search (贯穿科研全流程)

Literature retrieval runs throughout all research phases — not just Phase 1:

### Phase 1: Broad Search (Initial Literature Review)
- **Broad search**: 15–30 papers for literature review
- **Autonomous PDF download**: Attempt automatic retrieval (arXiv, open access, APIs)
- **User assistance**: Request help for paywalled papers
...
### Phase 1 Deep Thinking: On-Demand Verification
- **Verify insights**: Search to validate thinking strategies
- **Cross-domain methods**: Find techniques from other fields
- **Contradiction search**: Look for opposing views
...
### Phase 3: Design-Driven Search
- **New questions**: Search when design raises new questions
- **Baseline discovery**: Find comparison methods
- **Implementation details**: Search for technical approaches

```bash
python scripts/paper_search.py "method OR technique name" --sources semantic,arxiv --max 15
```

### Phase 4: Context Search for Unexpected Results
- **Explain anomalies**: Find papers that explain unexpected results
- **Method troubleshooting**: Understand why something didn't work
- **Alternative approaches**: Find backup strategies

```bash
python scripts/paper_search.py "unexpected result explanation" --max 10
```

### Phase 5–6: Citation Support for Paper Claims
- **Support claims**: Find citations for specific statements
- **Related work**: Comprehensive literature for Introduction
- **Position findings**: Locate papers for comparison
...
1. **Autonomous retrieval first**: Try arXiv, open access, APIs
2. **Mark status**: Track as `abstract_only`, `download_failed`, or `user_provided`
3. **Request user help**: Provide clear instructions for inaccessible papers
...
8. **Continuous literature search** across all research phases (Phase 1-6)
9. **Deep reading capability** with full PDF text extraction
10. **Full abstracts** - no truncation by default for complete information
11. **Graceful degradation** - never fabricates citations
