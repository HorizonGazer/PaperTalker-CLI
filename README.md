...
编辑 [video.md](video.md) 文件，修改视频生成提示词。

**默认提示词**：
```
请用简体中文制作，严格忠实原文：必须引用文献中的具体数值、统计结果、置信区间、p值、
准确率、AUC等指标，必要时展示原文真实图表（要清晰）以及所有相关公式和数学表达式。
讲清研究背景、方法、主要发现与局限时，每个关键点都要有数据支撑，不得使用模糊表述
（如'显著提升'、'效果良好'），必须给出具体数值。保持科研严谨，有专业深度，禁止
泛泛而谈或空洞概括。面向相关领域研究者或高年级学生。
...
1. **科普向**（面向大众）：
```
请用简体中文制作，用通俗易懂的语言解释复杂概念，多用类比和生活化例子。
保持科学准确性的同时，降低专业术语密度，适合对该领域感兴趣的普通观众。
```

2. **教学向**（面向学生）：
```
请用简体中文制作，按照"是什么-为什么-怎么做"的逻辑展开。重点讲解核心概念、
基本原理和关键方法，配合图表和公式详细说明。适合本科生或研究生学习使用。
```

3. **综述向**（面向研究者）：
```
请用简体中文制作，系统梳理该领域的研究脉络、主要方法和最新进展。对比不同
研究的优缺点，指出未解决的问题和未来方向。引用具体数据和实验结果。
```
...
修改后，使用 `--instructions` 参数指定自定义提示词文件：
```bash
python quick_video.py "主题" --instructions my_prompt.md
```
